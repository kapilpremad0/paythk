...
router.post('/', adminAuth, upload.fields([{ name: "profile", maxCount: 1 }]), hostelController.storeData);
router.get('/create', adminAuth, hostelController.create);
router.get('/:id', adminAuth, hostelController.getDetail);
router.put('/:id', adminAuth,upload.fields([{ name: "profile", maxCount: 1 }]), hostelController.updateData);
